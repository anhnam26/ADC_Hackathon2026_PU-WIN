...
import { addDays, createTasks, officeDate, ownerFor, transitionTask } from '../../src/lib/taskRules';
...
function sample(overrides: Partial<Issue> = {}): Issue {
  return { id: 'issue-1', sessionId: 'session-1', stepId: 0, locationId: 'side-entry',
    category: 'entrance', kind: 'verification', impact: 'normal', description: 'Xác minh giờ mở lối bên hông.', requestedSupport: '', state: 'draft',
...
describe('assignments and idempotency', () => {
  it('assigns physical barriers to Facilities and meeting support to HR', () => {
    ...
    expect(ownerFor(sample({ category: 'guidance', locationId: 'quiet' }))).toBe('HR');
  });
  it('preserves distinct issues at one location and does not duplicate resubmissions', () => {
    const issues = [sample(), sample({ id: 'issue-2', impact: 'high', description: 'Cần xác minh độ rộng cửa.' })];
  ...
  it('uses office timezone and handles month/year/leap boundaries without silently extending overdue dates', () => {
    ...
    expect(createTasks([sample()], [], '2020-01-01')[0].dueDate).toBe('2019-12-31');
  });
});
describe('preparation lifecycle', () => {
  ...
  it('requires preparation and employee review, with history preserved', () => {
    ...
    expect(() => transitionTask(task, 'ready_for_review', 'HR/Facilities', 'Sẵn sàng')).toThrow();
    const started = transitionTask(task, 'in_progress', 'HR/Facilities');
    expect(() => transitionTask(started, 'ready_for_review', 'HR/Facilities', '  ')).toThrow();
    const review = transitionTask(started, 'ready_for_review', 'HR/Facilities', 'Mở cửa từ 08:00, lễ tân hỗ trợ.');
    expect(() => transitionTask(review, 'done', 'HR/Facilities')).toThrow();
    expect(() => transitionTask(review, 'in_progress', 'Nhân viên', '')).toThrow();
    const reopened = transitionTask(review, 'in_progress', 'Nhân viên', 'Tôi cần đến lúc 07:30.');
    expect(reopened.employeeResponse).toContain('07:30');
    const revised = transitionTask(reopened, 'ready_for_review', 'HR/Facilities', 'Đã bố trí mở từ 07:15.');
    const done = transitionTask(revised, 'done', 'Nhân viên');
    expect(done.status).toBe('done');
    expect(done.history).toHaveLength(6);
    expect(() => transitionTask(done, 'in_progress', 'HR/Facilities')).toThrow();
  });
});
describe('saved sessions and route references', () => {
  ...
  it('rejects malformed, outdated, invalid dates and broken references', () => {
    ...
    expect(() => parseSession(JSON.stringify({ ...s, schemaVersion: 99 }))).toThrow();
    expect(() => parseSession(JSON.stringify({ ...s, startDate: '2026-02-31' }))).toThrow();
    expect(() => parseSession(JSON.stringify({ ...s, issues: [sample({ sessionId: s.id, locationId: 'missing' })] }))).toThrow();
    expect(() => parseSession(JSON.stringify({ ...s, tasks: createTasks([sample()], [], s.startDate) }))).toThrow();
    expect(() => parseSession(JSON.stringify({ ...s, answers: { missing: 'barrier' } }))).toThrow();
  });
  it('every journey and hotspot has an available destination and finite route', () => {
    ...
    for (const step of journey) expect(locations.some(l => l.id === step.locationId)).toBe(true);
    for (const location of locations) for (const stepFree of [false, true]) {
