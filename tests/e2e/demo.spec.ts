...
test("end-to-end: two issues, idempotent submit, HR response, employee review, persistence and reset", async ({
  page,
}) => {
  const errors: string[] = [];
  ...
  await page
    .getByLabel("Lối vào không có bậc thang", { exact: true })
    .selectOption("needs_verification");
  await page.getByRole("button", { name: "Lưu vào tổng kết" }).click();
  await page.getByRole("button", { name: /09:30/ }).click();
  await page
    .getByLabel("Có phụ đề hoặc tài liệu chữ", { exact: true })
    .selectOption("needs_verification");
  await page.getByRole("button", { name: "Lưu vào tổng kết" }).click();
  ...
  await expect(
    page.getByRole("button", { name: "Tạo 0 nhiệm vụ chuẩn bị" }),
  ).toBeDisabled();
  await page
    .getByRole("combobox", { name: "Vai trải nghiệm" })
    .selectOption("hr");
  ...
  await page
    .getByRole("button", {
      name: "Cần xác nhận phụ đề được bật trong buổi họp chào mừng.",
      exact: true,
    })
    .click();
  await page.getByRole("button", { name: "Bắt đầu chuẩn bị" }).click();
  await page
    .getByLabel("Phương án chuẩn bị", { exact: true })
    .fill("Đã chuẩn bị phụ đề và gửi tài liệu chữ trước buổi họp.");
  ...
  await page
    ...
    .click();
  await page
    .getByRole("button", { name: "Xác nhận phương án", exact: true })
    .click();
...
test("3D loads, moving avatar, office scene, screenshot and no runtime errors", async ({
  page,
}) => {
  const errors: string[] = [];
  page.on("pageerror", (e) => errors.push(e.message));
  ...
  await expect(
    page.getByRole("button", { name: "Đến địa điểm", exact: true }),
  ).toBeVisible();
  await page.screenshot({
    path: "test-results/dayzero-desktop.png",
    fullPage: true,
  });
  await page.getByRole("button", { name: "Đến địa điểm", exact: true }).click();
  await expect(page.locator(".avatar-label")).toBeVisible();
...
test("fallback, mobile layout, dialog cancellation, edit and delete drafts", async ({
  page,
}) => {
  ...
  await page
    .getByLabel("Lối vào không có bậc thang", { exact: true })
    .selectOption("barrier");
  await page.getByRole("button", { name: "Hủy", exact: true }).click();
  await expect(
    page.getByLabel("Lối vào không có bậc thang", { exact: true }),
  ).toHaveValue("");
  await record(page, "Cần biết giờ mở lối bên hông.");
  await page.getByRole("button", { name: "Tổng kết trải nghiệm" }).click();
  await page
    .getByRole("button", { name: "Sửa: Cần biết giờ mở lối bên hông." })
    .click();
  await page
    .getByLabel("Điều bạn muốn ghi nhận")
    .fill("Cần biết giờ mở cửa lúc 07:30.");
  await page.getByRole("button", { name: "Lưu vào tổng kết" }).click();
  await expect(page.locator(".issue-row")).toContainText("07:30");
  await page
    .getByRole("button", { name: "Xóa: Cần biết giờ mở cửa lúc 07:30." })
    .click();
  await expect(page.locator(".issue-row")).toHaveCount(0);
  await page.getByRole("button", { name: "Hành trình ngày đầu" }).click();
  for (let i = 0; i < 6; i++)
    await page
      .getByRole("button", { name: "Hoàn thành chặng", exact: true })
      .click();
  await page
    .getByRole("button", { name: "Hoàn thành & tổng kết", exact: true })
    .click();
  ...
  await page.getByRole("button", { name: "Hành trình ngày đầu" }).click();
  await page.screenshot({
    path: "test-results/dayzero-mobile.png",
    fullPage: true,
  });
});
...
test("corrupt storage stays untouched until reset", async ({ page }) => {
  await page.addInitScript(() => {
    localStorage.setItem("dayzero.session.v1", "{corrupt");
  });
  await start(page);
  ...
  expect(
    await page.evaluate(() => localStorage.getItem("dayzero.session.v1")),
  ).toBe("{corrupt");
  await page.getByRole("button", { name: "Tổng kết trải nghiệm" }).click();
  await expect(
    page.getByText("Chưa có điều gì cần chuẩn bị thêm"),
  ).toBeVisible();
});
