...
test('cabins carry the player through real height without changing the canvas, then permit manual exit',async({page})=>{
 ...
 await stage.focus();await expect(page.locator('.interact-button')).toContainText('Thang máy');await page.keyboard.press('f');
 await expect(page.getByRole('button',{name:'Đi đến Tầng 2',exact:true})).toHaveCount(0);
 await page.getByRole('button',{name:'Gọi thang / Mở cửa',exact:true}).click();await expect(stage).toHaveAttribute('data-lift-phase','open');
 await hold(page,'w',async()=>Number(await stage.getAttribute('data-x'))<-9.85);
 await page.waitForTimeout(150);await page.keyboard.press('f');await page.getByRole('button',{name:'Đi đến Tầng 2',exact:true}).click();
 ...
 await page.getByRole('button',{name:'Đi đến Tầng 1',exact:true}).click();await expect(stage).toHaveAttribute('data-floor','1',{timeout:20000});await expect(stage).toHaveAttribute('data-lift-phase','open');
