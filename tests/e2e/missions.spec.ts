...
test('manager assigns a mission; employee drives manually and receives automatic review',async({page,browser})=>{
 ...
 try{
   ...
   await editor.getByLabel('Starting point',{exact:true}).selectOption('ground-lobby');
   await editor.getByRole('button',{name:'Remove checkpoint 2',exact:true}).click();
   await editor.getByLabel('Final destination',{exact:true}).selectOption('f2-water');
   ...
   for(const point of path.slice(1)){
     ...
     const distance=Math.hypot(point.x-p.x,point.z-p.z);if(distance<.15)continue;
     const heading=Math.atan2(p.x-point.x,p.z-point.z),diff=Math.atan2(Math.sin(heading-p.yaw),Math.cos(heading-p.yaw));
     const key=diff>0?'a':'d';await stage.focus();await page.keyboard.down(key);await page.waitForTimeout(Math.abs(diff)/1.8*1000);await page.keyboard.up(key);await page.waitForTimeout(120);
     await page.keyboard.down('w');await page.waitForTimeout(distance/2.8*1000);await page.keyboard.up('w');await page.waitForTimeout(250);
   }
