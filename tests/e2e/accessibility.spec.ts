...
test('game and object information are accessible by keyboard; storage failures do not block use', async ({ page }) => {
  ...
  await page.goto('/'); await page.getByRole('button', { name: "Start exploring", exact: true }).focus(); await page.keyboard.press('Enter');
  await expect(page.locator('.storage-notice')).toContainText('Browser storage is unavailable');
...
test('corrupt old session is preserved instead of overwritten', async ({ page }) => {
  ...
  await page.goto('/'); await page.getByRole('button', { name: "Start exploring", exact: true }).click();
  await expect(page.locator('.storage-notice')).toContainText('Saved data could not be restored');
