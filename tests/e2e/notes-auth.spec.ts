...
test('login, notes at any location, manager map review and employee response across sessions',async({page,browser})=>{
 ...
 const manager=await browser.newContext({baseURL:'http://127.0.0.1:5173'});const admin=await manager.newPage();
 try{
 ...
 await admin.getByLabel('Kết quả xem xét',{exact:true}).selectOption('approved');await admin.getByLabel('Đánh giá và phương án',{exact:true}).fill('Sẽ bố trí mái che và mở rộng điểm dừng trước ngày nhận việc.');await admin.getByRole('button',{name:'Lưu đánh giá',exact:true}).click();await expect(admin.getByRole('status')).toContainText('Đã lưu đánh giá');
