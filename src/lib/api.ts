...
export async function api<T>(path:string,options:RequestInit={}):Promise<T>{
  ...
  let data;try{data=await response.json();}catch{throw new Error('Máy chủ chưa sẵn sàng. Hãy chạy npm run dev hoặc npm run preview.');}
  if(!response.ok){if(response.status===401&&!path.includes('/auth/login'))window.dispatchEvent(new Event('dayzero-auth-expired'));throw new Error(data.error||'Không thể kết nối máy chủ.');}return data;
}
