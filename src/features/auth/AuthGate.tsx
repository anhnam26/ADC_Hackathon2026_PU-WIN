...
export default function AuthGate(){
  ...
  if(user.role!=='manager'&&location.pathname==='/admin')return <main className="auth-page"><section><h1>{vi?'Trang dành cho quản lý':'Manager access required'}</h1><p>{vi?'Tài khoản của bạn có quyền trải nghiệm và gửi ghi chú.':'Your account can explore the office and submit notes.'}</p><button className="button primary" onClick={openExperience}>{vi?'Vào trải nghiệm':'Enter simulator'}</button></section></main>;
