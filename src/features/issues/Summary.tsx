  ...
  return (
    <div className="page-content">
      <div className="page-intro">
        <span className="eyebrow">SAU MỘT VÒNG LÀM QUEN</span>
        <h1>Một ngày đầu được chuẩn bị tốt hơn.</h1>
        <p>
          Những điều bạn ghi nhận sẽ trở thành việc chuẩn bị cụ thể cho HR và
          Facilities.
        </p>
      </div>
      <div className="stat-grid">
        <div className="stat-card">
          <span>Chặng đã trải nghiệm</span>
          <strong>
            {completed}
            <small> / 7</small>
          </strong>
          <span>Không phải đánh giá mức đáp ứng</span>
        </div>
        <div className="stat-card">
          <span>Điều cần chuẩn bị</span>
          <strong>{session.issues.length.toString().padStart(2, "0")}</strong>
          <span>{draft.length} ghi nhận chưa tạo nhiệm vụ</span>
        </div>
        <div className="stat-card accent">
          <span>Phương án chờ bạn xem</span>
          <strong>
            {session.tasks
          ...
          <button className="text-button" onClick={onTasks}>
            Xem phản hồi <ArrowRight size={15} />
          </button>
        </div>
      </div>
      <section className="surface">
        <div className="section-heading">
          <h2>
            Những điều bạn đã ghi nhận{" "}
            <span className="count">{session.issues.length}</span>
          </h2>
          <span className="muted small">Chỉ lưu trong trình duyệt này</span>
        </div>
        {!session.issues.length ? (
          <div className="empty-state">
            <ClipboardCheck size={34} />
            <h3>Chưa có điều gì cần chuẩn bị thêm</h3>
            <p>Bạn có thể hoàn thành trải nghiệm mà không cần tạo ghi nhận.</p>
          </div>
        ) : (
                ...
                <article className="issue-row" key={issue.id}>
                  ...
                  <div className="issue-copy">
                    ...
                    <span className={`badge ${task ? task.status : "draft"}`}>
                      {task ? statusLabels[task.status] : "Chưa gửi"}
                    </span>
                    {task && (
                      <span className="muted small"> · {task.ownerTeam}</span>
                    )}
                  </div>
                  {issue.state === "draft" && (
                    <div className="row-actions">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          notify("Đã xóa ghi nhận chưa gửi.");
                        }}
                      >
                        <Trash2 size={17} />
                      </button>
        ...
        <div className="summary-actions">
          <p>
            <Check size={17} />
            Bạn có thể sửa ghi nhận trước khi tạo nhiệm vụ.
          </p>
          <button
          ...
          >
            <Send size={16} />
            Tạo {draft.length} nhiệm vụ chuẩn bị
          </button>
        </div>
      </section>
