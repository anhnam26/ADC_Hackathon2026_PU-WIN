...
export default function ObjectInspector({
  ...
  return (
    <Dialog
      title={object.name}
      subtitle="Hồ sơ đối tượng · Số đo lấy từ mô hình đang trải nghiệm"
      onClose={onClose}
      wide
    >
      <div className="inspector-grid">
        <div>
          ...
          <p className="illustration-note">
            Hình chi tiết được tạo từ cùng hình học với vật trong văn phòng. Góc
            nhìn được phóng to để đọc rõ.
          </p>
        </div>
        <div className="object-description">
          ...
          {object.clearWidth && (
            <div className="specific-measure">
              ...
              <span>
                Rộng thông thủy
                <strong>{Math.round(object.clearWidth * 100)} cm</strong>
              </span>
            </div>
          )}
          {object.underHeight && (
            <div className="specific-measure">
              <Ruler size={17} />
              <span>
                Khoảng trống dưới mặt
                <strong>
                  {Math.round(object.underHeight * 100)} cm cao ×{" "}
                  {Math.round((object.underWidth ?? 0) * 100)} cm rộng
                </strong>
              </span>
            </div>
          )}
          {object.controlHeight && (
            <div className="specific-measure">
              <Ruler size={17} />
              <span>
                {object.kind === "door" ? "Cao tay nắm" : "Cao vị trí sử dụng"}
                <strong>
                  {Math.round(object.controlHeight * 100)} cm từ sàn
                </strong>
              </span>
            </div>
          )}
          {profile.mode === "wheelchair" && difference !== null && (
            ...
            >
              <strong>
                {difference <= 0
                  ? "Xe không lọt ô cửa theo số đo"
                  : difference < 12
                    ? "Ô cửa khá sát với xe của bạn"
                    : "Ô cửa rộng hơn xe của bạn"}
              </strong>
              <p>
                Ô cửa {Math.round(object.clearWidth! * 100)} cm − xe{" "}
                {profile.widthCm} cm = {difference} cm{" "}
                {difference > 0 ? "dư tổng cộng" : "chênh lệch"}.
              </p>
              <small>
                {difference > 0
                  ? `Khi căn giữa, khoảng dư mỗi bên khoảng ${(difference / 2).toFixed(1)} cm. `
                  : ""}
                Cần xét cả hướng tiếp cận, tay đẩy và đồ mang theo.
              </small>
            </div>
          )}
          {profile.mode === "wheelchair" && object.underHeight && (
            ...
            >
              <strong>Tay vịn của bạn: {profile.armrestHeightCm} cm</strong>
              <p>
                {profile.armrestHeightCm >= object.underHeight * 100
                  ? "Tay vịn cao hơn hoặc bằng khoảng trống dưới mặt. Có thể cần điều chỉnh."
                  : `Còn khoảng ${(object.underHeight * 100 - profile.armrestHeightCm).toFixed(0)} cm theo chiều cao.`}
              </p>
              <small>
                Chưa mô phỏng tư thế đưa chân vào dưới bàn hoặc tầm với cá nhân.
              </small>
            </div>
          )}
        </div>
